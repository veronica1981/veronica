...
using System.Data.SqlClient;
	...
	"Ferme_CCL.Ferme_CCL,Judete.Denloc, Fabrici.Nume as NumeFabrica, Fermier.Nume as NumeFermier "+
		...
		public int GetFermeId(string crot)
		{
			// Create Instance of Connection and Command Object
			SqlConnection myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["fccl2ConnectionString"].ConnectionString);
			SqlCommand myCommand = new SqlCommand("Portal_GetFermeID", myConnection);
			...
			SqlParameter parameterID = new SqlParameter("@CodBare", SqlDbType.VarChar, 7);
			parameterID.Value = crot;
			myCommand.Parameters.Add(parameterID);
        ...
        public int GetFermeExpl(string codexpl, out string numeFerma)
        {
			numeFerma = string.Empty;
			int idFerma = 0;
			if (!string.IsNullOrEmpty(codexpl))
			{
				// Create Instance of Connection and Command Object
				SqlConnection myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["fccl2ConnectionString"].ConnectionString);
				SqlCommand myCommand = new SqlCommand("SELECT ID, Nume FROM Ferme_CCL WHERE Cod = '" + codexpl + "'", myConnection);
				...
				try
				{
					if (reader.Read())
					{
						idFerma = reader.GetInt32(0);
						numeFerma = reader.GetString(1);
					}
				}
				catch (Exception ex)
				{
					logger.Fatal(string.Format("GetFermeExpl|cod:{0}|msg:{1}|stack:{2}", codexpl, ex.Message, ex.StackTrace));
				}
				finally
				{
					myConnection.Close();
		...
		public string GetFermeRasa(string crot)
		{
			// Create Instance of Connection and Command Object
			SqlConnection myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["fccl2ConnectionString"].ConnectionString);
			SqlCommand myCommand = new SqlCommand("Portal_GetFermeRasa", myConnection);

			// Mark the Command as a SPROC
			myCommand.CommandType = CommandType.StoredProcedure;

			// Add Parameters to SPROC
			SqlParameter parameterRasa = new SqlParameter("@Rasa", SqlDbType.NVarChar, 10);
			parameterRasa.Direction = ParameterDirection.Output;
			...
			if (parameterRasa.Value == DBNull.Value)
				return "";
			else
				return (string)parameterRasa.Value;		
		}

        public string GetFermeRasa(string crot,int fermaid)
        {
            // Create Instance of Connection and Command Object
            SqlConnection myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["fccl2ConnectionString"].ConnectionString);
            SqlCommand myCommand = new SqlCommand("Portal_GetFermeRasaBV", myConnection);
            ...
            SqlParameter parameterCod = new SqlParameter("@CodBare", SqlDbType.VarChar, 7);
            parameterCod.Value = crot;
            myCommand.Parameters.Add(parameterCod);

            SqlParameter parameterID = new SqlParameter("@FermaId", SqlDbType.Int, 4);
            parameterID.Value = fermaid;
            myCommand.Parameters.Add(parameterID);
        ...
        public string GetFermeCrot(string crot, int fermaid)
        {
            // Create Instance of Connection and Command Object
            SqlConnection myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["fccl2ConnectionString"].ConnectionString);
            SqlCommand myCommand = new SqlCommand("Portal_GetFermeCrot", myConnection);

            // Mark the Command as a SPROC
            myCommand.CommandType = CommandType.StoredProcedure;

            // Add Parameters to SPROC
            SqlParameter parameterCrotalia = new SqlParameter("@Crotalia", SqlDbType.NVarChar, 20);
            parameterCrotalia.Direction = ParameterDirection.Output;
            myCommand.Parameters.Add(parameterCrotalia);
            ...
            if (parameterCrotalia.Value == DBNull.Value)
                return "";
            else
                return (string)parameterCrotalia.Value;
        }
